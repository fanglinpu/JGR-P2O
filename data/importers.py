...
class ICVLImporter(DepthImporter):
    ...
    def __init__(self, basepath, useCache=True, cacheDir='./cache/', refineNet=None):
        ...
        self.default_cubes = {'Training_2014': (250, 250, 250),
                              'Testing_2014': (250, 250, 250)}
        self.sides = {'Training_2014': 'right', 'Testing_2014': 'right'}
    ...
    def loadSequence(self, seqName, Nmax=float('inf'), shuffle=False, rng=None, docom=False, cube=None):
        ...
        :param seqName: sequence name, e.g. Training_2014, Testing_2014
        ...
        pickleCache = '{}/icvl_{}.pkl'.format(self.cacheDir, seqName)
        if self.useCache:
        ...
        if seqName == 'Training_2014':
            label_path = os.path.join(self.basepath, 'Training_2014/labels.txt')
            label_list = [line for line in open(label_path)]
            com_path = '{}/icvl_center_3D_train_refined.txt'.format(self.basepath)
            com_list = [line for line in open(com_path)]
            assert len(com_list) == len(label_list)
            coms = []
            labels = []
            for i in range(len(com_list)):
                if not com_list[i].startswith('invalid'):
                    labels.append(label_list[i])
                    coms.append(com_list[i])
                    
            #coms = []
            #labels = []
            #for i in range(len(com_list)):
            #    if label_list[i].startswith('2014') and not com_list[i].startswith('invalid'):
            #        labels.append(label_list[i])
            #        coms.append(com_list[i])
            #coms_aug = []
            #labels_aug = []
            #for i in range(len(com_list)):
            #    if not label_list[i].startswith('2014') and not com_list[i].startswith('invalid'):
            #        labels_aug.append(label_list[i])
            #        coms_aug.append(com_list[i])
            #idx = np.linspace(0, len(coms_aug)-1, len(coms_aug))
            #np.random.seed(0)
            #np.random.shuffle(idx)
            #for i in range(len(idx)):
            #    labels.append(labels_aug[int(idx[i])])
            #    coms.append(coms_aug[int(idx[i])])
            ...
            for i in range(len(labels)):
                ...
                com_cache = coms[i].split(' ')
                com_ = np.asarray([float(j) for j in com_cache])
                com2D = self.joint3DToImg(com_)
                
                label_cache = labels[i].split(' ')             
                dptFileName = os.path.join(self.basepath, 'Training_2014', label_cache[0])
                ...
                gtorig = np.asarray([float(j) for j in label_cache[1:]])
                gtorig.shape = (-1,3)
                
                # joints in 3D coordinates
                ...
                try:
                    ...
                    # dpt, M, com = hd.cropArea3D(com=None, size=config['cube'], docom=docom, dsize=(96,96))
                except UserWarning:
                ...
                data.append(DepthFrame(dpt.astype(np.float32), gtorig, gtcrop, M, gt3Dorig, gt3Dcrop, com2D, com3D, dptFileName,
        ...
        elif seqName == 'Testing_2014':
            label_path_1 = os.path.join(self.basepath, 'Testing_2014/test_seq_1.txt') 
            labels_1 = [line for line in open(label_path_1) if line.startswith('test')]
            label_path_2 = os.path.join(self.basepath, 'Testing_2014/test_seq_2.txt') 
            labels_2 = [line for line in open(label_path_2) if line.startswith('test')]
            labels = labels_1 + labels_2
            com_path = '{}/icvl_center_3D_test_refined.txt'.format(self.basepath)
            coms = [line for line in open(com_path)]
            assert len(coms) == len(labels)
            print('%s: %d sequences in total'%(seqName, len(labels)))
            ...
            for i in range(len(labels)):
                ...
                label_cache = labels[i].split(' ')
                dptFileName = os.path.join(self.basepath, 'Testing_2014', label_cache[0])
        ...
        for subdir in subdirs:
            ...
            for i in range(nImgs):
                ...
                try:
                    ...
                    dpt, M, com = hd.cropArea3D(com=com2D, size=cube_, docom=docom,dsize=(96,96))
                    # dpt, M, com = hd.cropArea3D(com=None, size=cube_, docom=docom,dsize=(96,96))
                ...
                data.append(DepthFrame(dpt.astype(np.float32), gtorig, gtcrop, M, gt3Dorig, gt3Dcrop, com2D, com3D,
...
class NYUImporter(DepthImporter):
    ...
    def __init__(self, basepath, useCache=True, cacheDir='./cache/', refineNet=None, allJoints=False):
        """
        Constructor
        :param basepath: base path of the NYU dataset
        :return:
        """
    ...
    def loadSequence(self, seqName, Nmax=float('inf'), shuffle=False, rng=None, docom=False, cube=None, hand=None):
        ...
        # print (trainlabels)
        comlabels='{}/{}_center_NYU.txt'.format(self.basepath,seqName)
        f = open(comlabels, 'r')
        coms=f.readlines()
        # print (type(coms[0]))
